...
def run():
    ...
    try:
        ...
        print(f"[Check-out] {len(bookings)} booking(s) checking out {tomorrow}")
        for b in bookings:
            ...
            if not owner:
                continue
            msg = (
                f"Hi {owner.name}! Reminder: *{pet.name}*'s stay at Bejaa Pet Hotel ends "
                f"tomorrow ({tomorrow.strftime('%d %b %Y')}). "
                f"Please collect them by 12:00 PM. Thank you for choosing us! 🐾"
            )
            if send_whatsapp(owner.phone, msg):
                sent += 1
    finally:
        db.close()
    print(f"[Check-out] Done. {sent}/{len(bookings)} sent.\n")
